...
// 				}
		...
		// compute pipeline
		{
			...
			m_pipeline[Pipeline_MakeHitpoint] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[0]).value;
			m_pipeline[Pipeline_RayMarch] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[1]).value;
			m_pipeline[Pipeline_RayBounce] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[2]).value;
			m_pipeline[Pipeline_MakeDirectLight] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[3]).value;
			m_pipeline[Pipeline_PixelBasedRaytracing] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[4]).value;
			m_pipeline[Pipeline_PixelBasedRaytracing2] = context->m_device.createComputePipelineUnique(vk::PipelineCache(), compute_pipeline_info[5]).value;
		}
		...
		// graphics pipeline
		{
			...
			m_pipeline[Pipeline_DirectLighting] = context->m_device.createGraphicsPipelineUnique(vk::PipelineCache(), graphics_pipeline_info[2]).value;
			m_pipeline[Pipeline_LBR_MakeVPL] = context->m_device.createGraphicsPipelineUnique(vk::PipelineCache(), graphics_pipeline_info[3]).value;
		}
	...
	void executeLightBasedRaytracing(const vk::CommandBuffer& cmd)
	{
		...
		// ライトデータ作成
		static std::array<Emissive, 4> s_data;
		{
