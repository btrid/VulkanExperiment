...
struct PathFinding
{
	...
	PathFinding(const Description& desc)
	{
		cStopWatch time;
		m_desc = desc;
		auto aa = m_desc.m_size >> 3;
		m_field.resize(aa.x*aa.y);
	...
	void write()
	{
		auto map_size = m_desc.m_size >> 3;
		FILE* file;
		fopen_s(&file, "test.txt", "w");
		for (uint32_t y = 0; y < m_desc.m_size.y; y++)
		{
			std::vector<char> output(m_desc.m_size.x + 1);
			for (uint32_t x = 0; x < m_desc.m_size.y; x++)
			{
				ivec2 m = ivec2(x, y) >> 3;
				ivec2 c = ivec2(x, y) - (m << 3);
				output[x] = (m_field[m.x + m.y*map_size.x] & (1ull << (c.x + c.y * 8))) != 0 ? '#' : ' ';
			}
			output[m_desc.m_size.x] = '\0';
			fprintf(file, "%s\n", output.data());
		}
		fclose(file);
	}
