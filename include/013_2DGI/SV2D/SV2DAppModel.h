...
struct SV2DAppModel : public SV2DPipeline
{
	enum Shader
	{
		ShaderRenderingVS,
	...
	SV2DAppModel(const std::shared_ptr<btr::Context>& context, const std::shared_ptr<SV2DContext>& sv2d_context)
	{
		m_context = context;
		...
		// レンダーパス
		{
			...
			framebuffer_info.setWidth(sv2d_context->RenderWidth);
			framebuffer_info.setHeight(sv2d_context->RenderHeight);
		...
		// pipeline
		{
			...
			vk::Extent3D size = sv2d_context->RenderWidth;
			// viewport
			vk::Viewport viewport = vk::Viewport(0.f, 0.f, (float)sv2d_context->RenderWidth, (float)sv2d_context->RenderHeight, 0.f, 1.f);
			vk::Rect2D scissor = vk::Rect2D(vk::Offset2D(0, 0), vk::Extent2D(sv2d_context->RenderWidth, sv2d_context->RenderHeight));
	...
	vk::UniqueCommandBuffer createCmd(const std::shared_ptr<AppModel>& render)
	{
		...
		auto cmd = std::move(m_context->m_device->allocateCommandBuffersUnique(cmd_buffer_info)[0]);
		{
			...
			std::vector<vk::DescriptorSet> sets = {
				m_sv2d_context->getDescriptorSet(),
