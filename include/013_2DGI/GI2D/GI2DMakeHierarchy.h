...
struct GI2DMakeHierarchy
{
	...
	void executeMakeSDF(vk::CommandBuffer cmd, const std::shared_ptr<GI2DSDF>& sdf_context)
	{
		...
		// make sdf
		{
			...
			cmd.bindPipeline(vk::PipelineBindPoint::eCompute, m_pipeline[Pipeline_MakeJFA].get());
			{
				auto num = app::calcDipatchGroups(uvec3(sdf_context->m_gi2d_context->RenderWidth, sdf_context->m_gi2d_context->RenderHeight, 1), uvec3(16, 4, 1));
...
//			std::array<i16vec2, 4> random;
		} constant{ path_context->m_gi2d_context->RenderSize, i16vec2(1, 0), {iter/10, 0},  {i16vec2(11, 11), i16vec2(1002, 1002), i16vec2(144, 844), i16vec2(855, 255)}, {i16vec2(765, 354), {923, 33}, {555, 555}} };
