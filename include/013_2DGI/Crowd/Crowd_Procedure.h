...
struct Crowd_Procedure
{
	...
	Crowd_Procedure(const std::shared_ptr<CrowdContext>& context, const std::shared_ptr<GI2DContext>& gi2d_context)
	{
		...
		// pipeline
		{
			...
			shader_info[10].setModule(m_shader_module[Shader_PathFinding].get());
			shader_info[10].setStage(vk::ShaderStageFlagBits::eCompute);
			shader_info[10].setPName("main");
			shader_info[11].setModule(m_shader_module[Shader_DrawField].get());
			...
			m_pipeline[Pipeline_PathFinding] = std::move(compute_pipeline[10]);
			m_pipeline[Pipeline_DrawField] = std::move(compute_pipeline[11]);
		}
	}
	...
	void executeMakeRay(vk::CommandBuffer cmd)
	{
...

		{
			...
			cmd.pushConstants<MakeRayConstant>(m_pipeline_layout[PipelineLayout_MakeRay].get(), vk::ShaderStageFlagBits::eCompute, 0,
				MakeRayConstant{ ivec2(m_gi2d_context->m_desc.Resolution.x), 2.f, m_context->m_crowd_info.ray_angle_num });

			auto num = app::calcDipatchGroups(uvec3(m_gi2d_context->m_desc.Resolution.x, m_context->m_crowd_info.ray_angle_num, m_context->m_crowd_info.frame_max), uvec3(64, 16, 1));
	...
	void executePathFinding(vk::CommandBuffer cmd)
	{
		...
		cmd.bindDescriptorSets(vk::PipelineBindPoint::eCompute, m_pipeline_layout[PipelineLayout_PathFinding].get(), 1, m_gi2d_context->getDescriptorSet(), {});
		{
			...
			vk::BufferMemoryBarrier to_write[] = {
				...
				m_context->b_segment_counter.makeMemoryBarrier(vk::AccessFlagBits::eIndirectCommandRead, vk::AccessFlagBits::eTransferWrite),
			};
			cmd.pipelineBarrier(vk::PipelineStageFlagBits::eDrawIndirect|vk::PipelineStageFlagBits::eComputeShader, vk::PipelineStageFlagBits::eTransfer, {},
...

		{
			...
			cmd.bindPipeline(vk::PipelineBindPoint::eCompute, m_pipeline[Pipeline_PathFinding].get());
			cmd.dispatchIndirect(m_context->b_segment_counter.getInfo().buffer, m_context->b_segment_counter.getInfo().offset);
		}
