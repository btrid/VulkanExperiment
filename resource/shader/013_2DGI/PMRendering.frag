...
//	FragColor = b_color[index.x + index.y * int(u_pm_info.m_resolution.x)]*10.;
	vec3 rgb = vec3(0.);
	rgb.rg += texture(s_color[0], vec3(in_data.texcoord, 0.)).rg*1000;
	rgb.b += texture(s_color[0], vec3(in_data.texcoord, 1.)).r*1000;
	rgb.rg += texture(s_color[1], vec3(in_data.texcoord, 0.)).rg*1000;
	rgb.b += texture(s_color[1], vec3(in_data.texcoord, 1.)).r*1000;
