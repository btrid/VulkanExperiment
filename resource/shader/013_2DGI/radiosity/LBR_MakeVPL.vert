...
//			uint vertex_num = in_flag.y;
	...
	int vpl_index = hit_pos.x+hit_pos.y*reso.x;
	if((atomicOr(b_vpl_duplicate[vpl_index*32+gl_DrawID/32], 1<<(gl_DrawID%32)) & 1<<(gl_DrawID%32)) == 0)
	{
		uint count = atomicAdd(b_vpl_count[vpl_index], 1);
		if(count < 64){ b_vpl_index[vpl_index*64+count] = uint16_t(gl_DrawID);}
	}
//	gl_Position = vec4(vec2(gl_Position.xy+0.5)/reso.xy * 2. -1., 0., 1.);
