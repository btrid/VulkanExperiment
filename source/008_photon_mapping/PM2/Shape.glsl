...
Triangle scaleTriangleV(in Triangle tri, vec3 factor) {
	// すげえ適当
...
Triangle scaleTriangle(in Triangle tri, float factor) {
...
bool isDegenerate(in Triangle t){
	...
	vec3 ab = t.b - t.a;
	vec3 ac = t.c - t.a;
	return length(cross(ab, ac)) < 0.000000001;
}
