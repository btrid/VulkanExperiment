...
struct VertexGPU {
	...
	VertexGPU() = default;
	VertexGPU(const glm::vec3& p, int m, const glm::vec2& t0)
		: mPosition(p)
		, mMaterialIndex(m)
		, m_texcoord(t0)
	{}
...
struct Buffer {
	...
	{
		{
			...
			glNamedBufferData(mBO[BO_NORMAL], normal.size() * sizeof(decltype(std::remove_reference<decltype(normal)>::type())::value_type), normal.data(), GL_STATIC_DRAW);
	...
	void draw()
	{
		...
		glMultiDrawElementsIndirect(GL_TRIANGLES, GL_UNSIGNED_INT, nullptr, m_cmd_num, sizeof(Command));
	}
private:
	...
	AlignData alignData(const ModelList &list)
	{
		...
		for (auto& model : list.mData)
		{
			...
			for (auto& mesh : model.mMesh)
			{
				...
				for (auto& _v : v)
				{
					vertex.emplace_back(_v.mPosition, _v.mMaterialIndex, _v.m_texcoord);
				...
				for (auto& _v : v)
				{
					max_ = glm::max(_v.mPosition.xyz(), max_);
					min_ = glm::min(_v.mPosition.xyz(), min_);
				}
				c.aabb_max = glm::vec4(max_, 1.f);
				c.aabb_min = glm::vec4(min_, 1.f);
...
extern std::unique_ptr<Scene> SetupScene();
