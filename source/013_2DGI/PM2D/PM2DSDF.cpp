...
PM2DSDF::PM2DSDF(const std::shared_ptr<btr::Context>& context, const std::shared_ptr<PM2DContext>& pm2d_context)
{
	m_context = context;
...
void PM2DSDF::execute(vk::CommandBuffer cmd)
{
...

	{
		...
		cmd.bindDescriptorSets(vk::PipelineBindPoint::eCompute, m_pipeline_layout[PipelineMakeSDF].get(), 0, m_pm2d_context->getDescriptorSet(), {});
		cmd.dispatch(1, 1, 1);
