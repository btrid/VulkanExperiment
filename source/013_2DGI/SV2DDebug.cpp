...
SV2DDebug::SV2DDebug(const std::shared_ptr<btr::Context>& context, const std::shared_ptr<SV2DContext>& sv2d_context)
{
	m_context = context;
	m_sv2d_context = sv2d_context;
	std::vector<SV2DContext::Fragment> map_data(sv2d_context->RenderWidth*sv2d_context->RenderHeight);
	{
...
#endif
		for (size_t y = 0; y < sv2d_context->RenderHeight; y++)
		{
			for (size_t x = 0; x < sv2d_context->RenderWidth; x++)
			{
				auto& m = map_data[x + y * sv2d_context->RenderWidth];
				m.albedo = vec3(0.f);
				for (auto& r : rect)
				{
	...
	btr::BufferMemoryDescriptorEx<SV2DContext::Fragment> desc;
	...
	// pipeline layout
	{
		...
		vk::PushConstantRange constants[] = {
			vk::PushConstantRange().setOffset(0).setSize(sizeof(SV2DLightData)).setStageFlags(vk::ShaderStageFlagBits::eCompute),
		};
...
void SV2DDebug::execute(vk::CommandBuffer cmd)
{
	vk::BufferMemoryBarrier to_write[] =
	{
		...
		m_sv2d_context->b_fragment_buffer.makeMemoryBarrier(vk::AccessFlagBits::eShaderRead, vk::AccessFlagBits::eTransferWrite),
	};
	...
	copy.setDstOffset(m_sv2d_context->b_fragment_buffer.getInfo().offset);
	copy.setSize(m_sv2d_context->b_fragment_buffer.getInfo().range);

	cmd.copyBuffer(m_map_data.getInfo().buffer, m_sv2d_context->b_fragment_buffer.getInfo().buffer, copy);
...
//	if(0)
	{
		...
		cmd.pushConstants<SV2DLightData>(m_pipeline_layout[PipelineLayoutPointLight].get(), vk::ShaderStageFlagBits::eCompute, 0, SV2DLightData{ light_pos, light_dir, -1.4f, vec4(2500.f, 0.f, 2500.f, 0.f), level });
		cmd.dispatch(1, 1, 1);
	}
	vk::BufferMemoryBarrier to_read[] =
	{
		...
		m_sv2d_context->b_fragment_buffer.makeMemoryBarrier(vk::AccessFlagBits::eTransferWrite, vk::AccessFlagBits::eShaderRead | vk::AccessFlagBits::eIndirectCommandRead),
	};
