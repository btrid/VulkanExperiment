...
// 		}
		...
		for (int i = 0; i < 600; i++) {
//			rect.emplace_back(Fragment{ ivec4{ std::rand() % gi2d_context->RenderWidth , std::rand() % gi2d_context->RenderHeight, std::rand() % 12 + 32, std::rand() % 12 + 32 }, vec4{ 0.8f,0.2f,0.2f,0.f } });
		}		
